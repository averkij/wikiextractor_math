...
def clean(extractor, text, expand_templates=False, html_safe=True):
    ...
    #FIX. Expand placeholders

    # Match HTML placeholder tags
    placeholder_tag_patterns_newline_formula = [
        (re.compile(r'\n<\s*%s(\s*| [^>]+?)>(.*?)<\s*/\s*%s\s*>' % (tag, tag), re.DOTALL | re.IGNORECASE), tag) for tag in placeholder_tags
    ]
    placeholder_tag_patterns_inline_formula = [
        (re.compile(r'<\s*%s(\s*| [^>]+?)>(.*?)<\s*/\s*%s\s*>' % (tag, tag), re.DOTALL | re.IGNORECASE), tag) for tag in placeholder_tags
    ]
    
    #FIX. disable math and code placeholders
    math_count = 0
    for pattern, placeholder in placeholder_tag_patterns_newline_formula:
        for match in pattern.finditer(text):
            text = text.replace(match.group(), f'\n$${match.group(2)}$$')
            math_count += 1
...
#FIX
# placeholder_tags = {'math': 'formula', 'code': 'codice'} #default

# leave code as <code>...</code> in an article
placeholder_tags = ['math']
